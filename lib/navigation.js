 ...
 * `facades` = niveaux de contexte qui voient le lien (region / commune / pme).
 * cf. DESIGN_SYSTEM.md §3.2 — garder aligné avec lib/facade-routes.js
 */
export const SECTIONS_NAV = [
  ...
  },
  {
    ...
    liens: [
      { to: '/dashboard/tarifs', label: 'Tarifs', disponible: true, facades: ['commune'] },
      { to: '/dashboard/paiements', label: 'Paiements', disponible: true, facades: ['commune', 'pme'] },
    ],
  },
  {
    id: 'signalements',
  ...
  },
  {
    ...
    titre: 'PME',
    domaine: 'PME',
    description: 'Entreprises de collecte, périmètres et agents.',
    Icon: IconImmeuble,
    liens: [
      { to: '/dashboard/pme', label: 'PME', disponible: true, facades: ['region', 'commune', 'pme'] },
      { to: '/dashboard/pme/perimetres', label: 'Affectation périmètre', disponible: true, facades: ['commune'] },
    ],
  },
  {
    id: 'administration',
...
export function liensVisiblesPour(ctx) {
  return SECTIONS_NAV.flatMap(function (s) {
    return s.liens.filter(function (l) {
      return l.disponible && lienVisible(l, ctx);
    });
  });
}
